...
def corner_detection(lines):
    ...
    if lines is not None:
        for i, line1 in enumerate(lines):
            ...
            for j, line2 in enumerate(lines):
                ...
                angleDifference = abs(calculate_line_angles(line1) - calculate_line_angles(line2))
    ...
    for sorted_lines in LinesSorted:
        ...
        for line in sorted_lines:
            if calculate_line_angles(line) >= 0:
...
# Function to remove close points using Euclidean distance
def remove_close_points(corners, threshold):
    unique_corners = []

    for point in corners:
        # Check if the point is close to any already added points
        if not any(euclidean_distance(point, existing_point) < threshold for existing_point in unique_corners):
            unique_corners.append(point)

    return np.array(unique_corners)

"""# process reference image"""

reference_image_path = '/content/drive/MyDrive/Image processing assignments /project/IMG-20241022-WA0001.jpg'
reference_image = cv2.imread(reference_image_path)

# Step 1: Show reference Image
...
ref_segmented_image = color_segmentation_green(reference_image)
plt.figure(figsize=(10, 10))
...
ref_edges, gray_image = detect_edges(ref_segmented_image)
plt.figure(figsize=(10, 10))
...
ref_dilated_edges = cv2.dilate(ref_edges, kernel, iterations=2)
ref_eroded_edges = cv2.erode(ref_dilated_edges, kernel, iterations=1)

# Show the results after morphological operations
plt.figure(figsize=(10, 10))
plt.title("Eroded Edges")
plt.imshow(ref_eroded_edges, cmap='gray')
plt.axis('off')
plt.show()

# Step 4: Detect lines using Hough Line Transform
ref_lines = detect_hough_lines(ref_eroded_edges)
ref_hough_image = reference_image.copy()
# Draw the detected Hough lines
hough_ref_image_with_lines = draw_lines(ref_hough_image, ref_lines)
plt.figure(figsize=(10, 10))
plt.title("Hough Lines Detected")
plt.imshow(cv2.cvtColor(hough_ref_image_with_lines, cv2.COLOR_BGR2RGB))
plt.axis('off')
plt.show()


# Step 5: Find the longest line
ref_longest_line = find_longest_line(ref_lines)

# Draw the longest line
if ref_longest_line is not None:
    x1, y1, x2, y2 = ref_longest_line[0]
    cv2.line(hough_ref_image_with_lines, (x1, y1), (x2, y2), (0, 0, 255), 5)  # Draw the longest line in red
# Show the image with the longest line
plt.figure(figsize=(10, 10))
...
ref_combined_image = find_and_draw_parallel_perpendicular(ref_hough_image, ref_lines, ref_longest_line)

# Step 7: Corner detection
ref_CornerPoints = corner_detection(ref_lines)

# Set a distance threshold to consider points as the same
threshold = 100.0  # Adjust this value as needed

# Remove duplicates and convert to numpy array
reference_corners = list(set(ref_CornerPoints))
reference_corners_np = np.float32(reference_corners).reshape(-1, 2)  # Shape (N, 2)

# Remove redundant close points
reference_corners_np = remove_close_points(reference_corners_np, threshold)

# Draw the corners and print their locations on the reference image
for point in reference_corners_np:
    cv2.circle(ref_combined_image, (int(point[0]), int(point[1])), radius=10, color=(255, 255, 255), thickness=-1)
    # Print the coordinates next to the point
    cv2.putText(ref_combined_image, f"{tuple(point)}", (int(point[0] + 10), int(point[1] - 10)),
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1, cv2.LINE_AA)

# Display the image with corners and their locations
plt.figure(figsize=(10, 10))
plt.imshow(cv2.cvtColor(ref_combined_image, cv2.COLOR_BGR2RGB))
plt.title("Reference Image with Corners and Locations")
plt.axis('off')
plt.show()
...
def main(input_video_file: str, output_video_file: str) -> None:
    ...
    while cap.isOpened():  # Loop until the video ends
        ...
        #Step 2: Masking out the outside area

        kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (6, 3))
        TestMask = cv2.erode(segmented_image, kernel, iterations=15)
        TestMask = cv2.cvtColor(TestMask, cv2.COLOR_BGR2GRAY)
        empty, TestMask = cv2.threshold(TestMask, 0, 255, cv2.THRESH_BINARY)
        TestMask = cv2.dilate(TestMask, kernel, iterations=30)
        TestMask = cv2.cvtColor(TestMask, cv2.COLOR_GRAY2BGR)
        segmented_image = cv2.bitwise_and(segmented_image, TestMask)
        ...
        # Step 6: Find the longest line
        longest_line = find_longest_line(lines)

        # Step 7: Draw combined image with both parallel and perpendicular lines
        ...
        # Remove duplicates and convert to numpy array
        target_corners = list(set(CornerPoints))
        target_corners_np = np.float32(target_corners).reshape(-1, 2)  # Shape (N, 2)

        #Remove redundant close points
        target_corners_np = remove_close_points(target_corners_np, threshold)

        # Draw the corners on the target image
        for point in target_corners_np:
          cv2.circle(combined_image, (int(point[0]), int(point[1])), radius=10, color=(255, 255, 255), thickness=-1)
          # Print the coordinates next to the point
          cv2.putText(combined_image, f"{tuple(point)}", (int(point[0] + 10), int(point[1] - 10)),
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1, cv2.LINE_AA)
